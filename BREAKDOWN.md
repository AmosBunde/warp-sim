...
Plan numbers (P1 and so on) are followed by the GitHub issue number in parentheses. Two issues were added during execution to keep pull requests within the size guideline (#20 assembler lexer, #19 reconvergence analysis) and one was added for the Python bindings (#30); the barrier planned as P18 was delivered with the scheduler in #29. Final state: 31 issues, each closed by a squash-merged pull request from a linked development branch.

...
- **Bank-conflict observable.** Shared memory has 32 banks of 4 bytes: word `w` lives in bank `w mod 32`. For one warp-level `ld.shared` or `st.shared`, the conflict degree is the maximum over banks of the number of distinct addresses that executing lanes direct at that bank; lanes that access the same address broadcast and count once. The per-launch counter `shared_wavefronts` is the sum of degrees (a degree-1 access costs one pass, a degree-32 access costs 32) and `shared_conflicted_accesses` counts accesses with degree above 1. These are counts, not times.
